...
public class ExampleEditor implements Editor, Listener {
	...
	public void postConstruct(final Composite parent) {
		...
		this.editorInput = (String) part.getTransientData().get(
				ExampleEditor.EDITOR_ID);

		part.setLabel(this.getLabel());

		parent.setLayout(new GridLayout(1, false));

		editorInputText = new Text(parent, SWT.BORDER | SWT.WRAP | SWT.H_SCROLL
				| SWT.V_SCROLL | SWT.CANCEL);
		editorInputText.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true,
				true, 1, 1));
		loadPersistedState();
	...
	private void persist() {
		try {
			File file = new File(URLDecoder.decode(
					Configuration.workspaceLocation.getPath(), "UTF-8")
					+ EDITOR_FILE);
	...
	private void loadPersistedState() {
		try {
			...
			while ((line = bR.readLine()) != null) {
				this.editorInputText.setText(this.editorInputText.getText()
						+ line + "\n");
			}
