...
public class GeneralSettingsPage {
	...
	public void postConstruct(
		...
		Font normalFont = resManager.createFont(FontDescriptor.createFrom(
		...
		header.setLayoutData(new GridData(SWT.LEFT, SWT.CENTER, true, false, 4,
				1));
		header.setText(resourceBundle.getString("generalSettingsHeader"));
		header.setFont(resManager.createFont(FontDescriptor.createFrom(
				Configuration.fontFamily, Configuration.fontSize, SWT.BOLD)));

		Label label = new Label(content, SWT.SEPARATOR | SWT.HORIZONTAL);
		label.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false, 4,
				1));

		Label languageLabel = new Label(content, SWT.NONE);
		languageLabel.setText(resourceBundle
				.getString("generalSettingsLanguageLabel"));
		languageLabel.setFont(normalFont);

		language = new Combo(content, SWT.READ_ONLY);
		language.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false,
				3, 1));
		...
		fontLabel.setLayoutData(new GridData(SWT.RIGHT, SWT.CENTER, false,
				false, 1, 1));
		...
		fontSizeLabel.setLayoutData(new GridData(SWT.RIGHT, SWT.CENTER, false,
				false, 1, 1));
		fontSizeLabel.setText(resourceBundle
				.getString("generalSettingsFontSizeLabel"));
		fontSizeLabel.setFont(normalFont);

		fontSize = new Combo(content, SWT.READ_ONLY);
		fontSize.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, false, false,
				1, 1));
		...
		restoreDefault.setLayoutData(new GridData(SWT.RIGHT, SWT.CENTER, true,
				false, 1, 1));
		restoreDefault.setText(resourceBundle
				.getString("generalSettingsRestoreDefaultButton"));
		restoreDefault.addListener(SWT.Selection, new Listener() {

			@Override
			public void handleEvent(final Event event) {
				broker.post(Events.TOPIC_PREFERENCES_GENERAL_RESTORE_DEFAULT,
						null);
			}
		});

		Button apply = new Button(footer, SWT.NONE);
		apply.setLayoutData(new GridData(SWT.RIGHT, SWT.CENTER, false, false,
				1, 1));
	...
	@Optional
	private void apply(
			@UIEventTopic(Events.TOPIC_PREFERENCES_GENERAL_APPLY) final Object o,
			@Preference(nodePath = PreferencesHandler.preferenceStore) final IEclipsePreferences prefs) {

		prefs.put(PreferenceKey.LANGUAGE, this.language.getText());
		prefs.put(PreferenceKey.FONT_FAMILY, this.font.getText());
		prefs.putInt(PreferenceKey.FONT_SIZE,
				new Integer(this.fontSize.getText()));
	...
	private void restoreDefault(
			@UIEventTopic(Events.TOPIC_PREFERENCES_GENERAL_RESTORE_DEFAULT) final Object o,
			@Preference(nodePath = PreferencesHandler.preferenceStore) final IEclipsePreferences prefs) {
